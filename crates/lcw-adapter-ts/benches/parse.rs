...
//!
//! Generates synthetic TypeScript source of increasing size (functions that
//! call a handful of their neighbours) and measures end-to-end
...
/// Build one `.ts` file with `fns` functions. Each function calls the next few
...
fn synth_source(fns: usize) -> String {
    ...
    for i in 0..fns {
        ...
        for step in 1..=3 {
            ...
            s.push_str(&format!("    acc += f{callee}(acc ^ {step});\n"));
        }
